...
public class SingletonFactory {
  ...
  public static BucketAllocator getBucketAllocator(FDSClientConfiguration fdsConfig) {
    BucketAllocatorKey key =
      new BucketAllocatorKey(fdsConfig.getPreReadBufferSize(), fdsConfig.getPreReadPartSize());
  ...
  public static ExecutorService getPreReadWorker(FDSClientConfiguration fdsConfig) {
    WorkerPoolKey key =
      new WorkerPoolKey(fdsConfig.getPreReadQueueSize(), fdsConfig.getPreReadPoolSize());
    if (SingletonHolder.INSTANCE.preReadWorkers.get(key) == null) {
      synchronized (SingletonHolder.INSTANCE.lock) {
        if (SingletonHolder.INSTANCE.preReadWorkers.get(key) == null) {
          ...
          ExecutorService preReadWorker =
            new ThreadPoolExecutor(fdsConfig.getPreReadPoolSize(), fdsConfig.getPreReadPoolSize(),
              60, TimeUnit.SECONDS, queue, new ThreadFactory() {
